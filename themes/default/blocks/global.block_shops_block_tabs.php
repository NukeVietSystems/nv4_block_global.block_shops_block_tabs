...
if( ! nv_function_exists( 'nv_block_shops_block_multi_tabs' ) )
{
	function nv_block_config_shops_block_multi_tabs( $module, $data_block, $lang_block )
	{
        ...
        if ($nv_Request->isset_request('loadajaxdata', 'get')) {
			...
			$sql = 'SELECT bid, '. NV_LANG_DATA .'_title, weight FROM ' . $db_config['prefix'] . '_shops_block_cat ORDER BY weight ASC';
			$list = $db->query( $sql);
			$html .= '<td>';
			while( $l = $list->fetch() )
			{
	...
	function nv_block_config_shops_block_multi_tabs_submit( $module, $lang_block )
	{
		global $nv_Request;
		$return = array();
		$return['error'] = array();
	...
	function nv_block_shops_block_multi_tabs( $block_config )
	{
		...
		if( file_exists( NV_ROOTDIR . '/themes/' . $global_config['module_theme'] . '/blocks/global.block_shops_block_tabs.tpl' ) )
		{
		...
		$xtpl = new XTemplate( 'global.block_shops_block_tabs.tpl', NV_ROOTDIR . '/themes/' . $block_theme . '/blocks' );
		$xtpl->assign( 'TEMPLATE', $block_theme);
		$xtpl->assign('LANG', $lang_module);
		$xtpl->assign( 'NVS_TITLE', $block_config['title']);
		$n = 0;
		$sql = 'SELECT bid, '. NV_LANG_DATA .'_title as title FROM ' . $db_config['prefix'] . '_shops_block_cat WHERE bid IN ( '.$blockid.' ) ORDER BY weight ASC' ;
		$result = $db->query( $sql );
		while( $data = $result->fetch( ) )
		{
			...
			$db->sqlreset()
				->select( 't1.id, t1.listcatid as catid, t1.'. NV_LANG_DATA .'_title as title, t1.'. NV_LANG_DATA .'_alias as alias, t1.homeimgfile, t1.homeimgthumb,t1.'. NV_LANG_DATA .'_hometext as hometext,t1.publtime, t1.showprice, t1.discount_id, t1.product_number, t1.money_unit, t1.product_code, t3.newday' )
				->from( $db_config['prefix'] . '_shops_rows t1' )
				->join( 'INNER JOIN ' . $db_config['prefix'] . '_shops_block t2 ON t1.id = t2.id LEFT JOIN ' . $db_config['prefix'] . '_shops_catalogs t3 ON t3.catid = t1.listcatid' )
			...
			if( ! empty( $list ) )
			{
				while( $l = $list->fetch() )
				{
					$l['link'] = NV_BASE_SITEURL . 'index.php?' . NV_LANG_VARIABLE . '=' . NV_LANG_DATA . '&amp;' . NV_NAME_VARIABLE . '=shops&amp;' . NV_OP_VARIABLE . '=' . $shops_array_cat[$l['catid']]['alias'] . '/' . $l['alias'] . $global_config['rewrite_exturl'];
					if( $l['homeimgthumb'] == 1 )
					{
...
if( defined( 'NV_SYSTEM' ) )
{
	...
	$shops_array_cat = array();
	$sql = 'SELECT catid, parentid, '. NV_LANG_DATA .'_title as title, '. NV_LANG_DATA .'_alias as alias, viewcat, subcatid, inhome, numlinks, '. NV_LANG_DATA .'_description as description, '. NV_LANG_DATA .'_keywords as keywords, groups_view FROM ' . $db_config['prefix'] . '_shops_catalogs ORDER BY sort ASC';
	$list = $db->query( $sql );
	while( $l = $list->fetch() )
	{
		$shops_array_cat[$l['catid']] = $l;
		$shops_array_cat[$l['catid']]['link'] = NV_BASE_SITEURL . 'index.php?' . NV_LANG_VARIABLE . '=' . NV_LANG_DATA . '&amp;' . NV_NAME_VARIABLE . '=shops&amp;' . NV_OP_VARIABLE . '=' . $l['alias'];
	}
	
	
	
	$content = nv_block_shops_block_multi_tabs( $block_config );
